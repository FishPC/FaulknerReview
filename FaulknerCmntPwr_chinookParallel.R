...
pwrOutDF<-data.frame(ssMult = numeric(),
                     totN = numeric(),
                     pwr = numeric(),
                     simN = numeric(),
                     stringsAsFactors = FALSE)

cl <- makeCluster(detectCores(all.tests = FALSE, logical = TRUE), type = "SOCK")     #"detectCores" call specifies the maximum number of logical processors on your machine; for less utilization (slower processing) change to a value to < output from "detectCores"
registerDoSNOW(cl)

start<-Sys.time()
for (ssMult in seq(1,9,1)){
  ss.multiplier=ssMult    #This is the sample size multiplier, ranging from 1 to 9; to generate Figure 2 in Storch et al. (in review); in this version of the code,  the multiplier will have to be adjusted manually
  sim.n=ss.multiplier*yearly.n
  tot.n=sum(sim.n)

  sims=200  #At small simulation sizes, output may vary slightly among runs (i.e., at a given miltiplier)
  Pr.z=rep(NA,sims)
  Pr.z.re=rep(NA,sims)
  ...
  out<-foreach(j=1:sims,
               .packages = "Hmisc",
               .combine=rbind) %dopar% {
                 pwr.fun(j)
               }

  pwr <- sum(out[,1]<.05)/sims #This is the estimated power for a given multiplier

  pwrOutDF[nrow(pwrOutDF)+1,] <- c(ssMult,
                                   tot.n,
                                   pwr,
                                   sims)
}
